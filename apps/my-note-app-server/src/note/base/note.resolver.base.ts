...
export class NoteResolverBase {
  ...
  async createNote(@graphql.Args() args: CreateNoteArgs): Promise<Note> {
    return await this.service.createNote({
      ...
      data: {
        ...
        notebook: args.data.notebook
          ? {
              connect: args.data.notebook,
            }
          : undefined,
      },
    });
  ...
  async getNotebook(@graphql.Parent() parent: Note): Promise<Notebook | null> {
    const result = await this.service.getNotebook(parent.id);
  ...
  @graphql.Mutation(() => CreateNoteInput)
  async CreateNoteWithAi(
    @graphql.Args()
    args: CreateNoteInput
  ): Promise<CreateNoteInput> {
    return this.service.CreateNoteWithAi(args);
  }
}
